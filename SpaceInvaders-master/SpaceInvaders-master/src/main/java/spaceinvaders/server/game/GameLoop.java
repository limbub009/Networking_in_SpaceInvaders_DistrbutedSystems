...
//SMOOTH MOVE
  ...
  for(int i = 0; i < steps; i++) { //run loop to the number of steps to smooth correct to initial position before delay
    int intermediateX = player.getX() + ((distance/steps) * direction);
    player.move(intermediateX, player.getY());
    Thread.sleep(100);
  }
}
  /**
